...
public interface FirmTransactionRepository extends CrudRepository<FirmTransaction, Long>{
	...
	@Query(value="select sum(amount) from FIRM_TRANSACTION a where a.FIRM_ID = ?1 and  upper(TRANSACTION_TYPE) = 'CREDIT' and TRANSACTION_DATE < ?2 ", nativeQuery = true)
    Optional<Double> findSumByTransactionDateTransactionTypeCredit(long firmId, Date startdate);	
	
	@Query(value="select sum(amount) from FIRM_TRANSACTION a where a.FIRM_ID = ?1 and  upper(TRANSACTION_TYPE) = 'DEBIT' and TRANSACTION_DATE < ?2 ", nativeQuery = true)
    Optional<Double> findSumByTransactionDateTransactionTypeDebit(long firmId, Date startdate);
}
